...
<article class="order-block">
    ...
    <?php foreach ($order['items'] as $item): ?>
        ...
        $skuDeleteFormId = 'drawer-image-delete-sku-' . (int) $item['id'];
        ?>
        <aside class="editor-drawer" id="editor-<?= e($item['id']) ?>" aria-hidden="true">
            ...
            <?php if ($canDeleteImage): ?>
                <?php foreach (['main' => $mainDeleteFormId, 'sku' => $skuDeleteFormId] as $imageKind => $imageFormId): ?>
                    <form id="<?= e($imageFormId) ?>" class="drawer-image-upload-form" method="post" action="/orders/images/delete">
                        <?= csrf_field() ?>
                        <input type="hidden" name="tenant" value="<?= e($tenantKey) ?>">
                        <input type="hidden" name="order_id" value="<?= e($order['id']) ?>">
                        <input type="hidden" name="item_id" value="<?= e($item['id']) ?>">
            ...
            <form class="drawer-body" method="post" action="/orders/item/save">
                ...
                <details class="drawer-section drawer-collapsible">
                    <summary class="drawer-section-title">编辑订单</summary>
                ...
                <div class="drawer-section">
                    ...
                    <div class="drawer-form-group">
                        ...
                        <div class="image-upload-area" tabindex="0" data-image-paste-area data-paste-target="<?= e($mainImageFormId) ?>" title="点击此处聚焦后可 Ctrl+V 粘贴图片">
                            ...
                            <?php endif; ?>
                            <?php if ($canUploadImage || ($canDeleteImage && $mainImage !== '')): ?>
                                <div class="drawer-image-controls">
                                    <?php if ($canUploadImage): ?>
                                    <label class="drawer-file-btn">
                                        <input form="<?= e($mainImageFormId) ?>" type="file" name="image" accept="image/*" data-image-file-input>
                                        <span>【选择图片】</span>
                                    </label>
                                    <input class="drawer-paste-input" type="text" readonly placeholder="可将图片粘贴到此" data-image-paste-input>
                                    <textarea form="<?= e($mainImageFormId) ?>" name="base64_image" rows="2" hidden data-image-base64></textarea>
                                    <?php endif; ?>
                                    <div class="drawer-image-actions">
                                        <?php if ($canUploadImage): ?>
                    ...
                    <div class="drawer-form-group">
                        ...
                        <div class="image-upload-area" tabindex="0" data-image-paste-area data-paste-target="<?= e($skuImageFormId) ?>" title="点击此处聚焦后可 Ctrl+V 粘贴图片">
                            ...
                            <?php else: ?>
                                <div class="drawer-image-empty">暂无图片</div>
                            <?php endif; ?>
                            <?php if ($canUploadImage || ($canDeleteImage && $skuImage !== '')): ?>
                                <div class="drawer-image-controls">
                                    <?php if ($canUploadImage): ?>
                                    <label class="drawer-file-btn">
                                        <input form="<?= e($skuImageFormId) ?>" type="file" name="image" accept="image/*" data-image-file-input>
                                        <span>【选择图片】</span>
                                    </label>
                                    <input class="drawer-paste-input" type="text" readonly placeholder="可将图片粘贴到此" data-image-paste-input>
                                    <textarea form="<?= e($skuImageFormId) ?>" name="base64_image" rows="2" hidden data-image-base64></textarea>
                                    <?php endif; ?>
                                    <div class="drawer-image-actions">
                                        <?php if ($canUploadImage): ?>
