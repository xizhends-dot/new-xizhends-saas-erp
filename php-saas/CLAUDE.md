...
- **导入导出**：CSV 导入解析预览、导出
- **权限**：基于角色（公司管理员/采购/客服/品检）+ 细粒度权限点 + 员工店铺范围过滤
...
最大的几个缺口：①除 Rakuten 外的平台订单同步（Wowma/Yahoo/Mercari/Qoo10/雅虎拍卖）全缺；②物流模块只有 UI 壳，1688/日本物流的真实 API 对接与 cron 自动同步未做；③订单详情字段不全（material/采购链接/1688单号等），需先补字段再做依赖它的品检、利润分摊等。新增/对照功能时请先查该清单，做完一项勾一项。
