...
- **存储驱动定位**：MySQL 是唯一生产驱动；`JsonStore` 仅作为本地演示与测试替身。
  - 新功能不再要求 `JsonStore` 与 MySQL 行为对齐，但被现有 `tests/*.php` 使用的方法必须保持可用。
  - 缺 `pdo_mysql` 扩展或 DSN 未配置时仍可回退 JSON，并在 `/admin/system` 显示原因。
...
修改数据层时记住：生产路径以 `MysqlStore` 为准；`JsonStore` 只承担本地演示和现有测试替身职责，新功能不再要求双驱动同步实现。

## AI 编码执行准则（对 Claude 与 Codex 均生效）

完整版见仓库根 [AGENTS.md](../AGENTS.md)（Karpathy 编码守则的本项目适配），核心四条：

1. **先想后写**：假设显式化，歧义列选项问用户，不静默挑一种理解；有更简单方案要说出来。
2. **最简实现**：只写解决当前问题的最少代码，不加未要求的功能/抽象/配置。
3. **外科手术式改动**：只动必须动的行；不顺手改无关代码；发现无关问题记录不改；
   结构搬移守"纯搬家铁律"。
4. **目标驱动**：动手前定可验证的成功标准，完成后逐条自验（lint→全量测试→登录态冒烟
   看内容与落库，不只看状态码）；自检不过不许交付。
